//crypto
// node 내장 모듈
const crypto = require('crypto'); //내장모듈이라 따로 설치할 필요없
...
const createHashedPassword = (password) => {
  //createHash('알고리즘 방식').update(암호화할_값).digest('인코딩_방식')
...
// console.log(createHashedPassword('1234')); //같은 pw라면 같은 해시값이 나온다.
...
function checkPassword(inputPassword, savedSalt, savedHash) {
  ...
  return savedHash === hash; // 같다면 true
}
...
const isMatch = checkPassword(inputPassword, salt, hash); //같다면 true
